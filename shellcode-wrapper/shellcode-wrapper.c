...
#define INCLUDE_EIP 0
#define INCLUDE_SYSC 0

typedef enum {HW, RAW, DUMP, RAWLONG} Source_type;
...
void execShellcodeRawlong (uint8_t* s, uint len, uint f, uint c, maddr_h eip, bool enable_multiple, uint32_t syscall);
...
int main (int argc, char* argv[]) {
   ...
   if (opts.exec_shellcode) {
      ...
      if (opts.source_type == RAWLONG) {
         execShellcodeRawlong (opts.shell, opts.shellcode.len, opts.offset, opts.cases, EIP_UNKNOWN, opts.multi_syscalls, opts.shellcode.syscall);
      }
      else {
...
void execShellcode (struct shellcode* s, uint f, uint c, bool eip_known, bool enable_multiple) {
   execShellcodeRawlong (s->shell, s->len, f, c, eip_known ? (maddr_h) s->shell + s->eip : EIP_UNKNOWN, enable_multiple, s->syscall);
   return;
}
...
void execShellcodeRawlong (uint8_t* s, uint len, uint f, uint c, maddr_h eip, bool enable_multiple, uint32_t syscall) {
   ...
   snprintf (buf, sizeof (buf), "<< Within execShellcode with params f: %u, c: %u, eip_known: %s, enable_multiple_syscalls: %s\n", f, c, (eip == EIP_UNKNOWN ? "no" : "yes"), (enable_multiple ? "yes" : "no") );
   #ifdef SYMB
   s2e_message (buf);
   #else
   printMemRange (s, len); // goes to stdout btu we want at least 1 copy in the debug txt file
   #endif
   
   #ifdef SYMB_OFFSET
   unsigned int i;
   for (i = f; i < (f + c); i++) {
      curr_offset = (maddr_h) s2e_codext_createFork (i);
   #else
      curr_offset = f;
   {
   #endif
      if (curr_offset != 0xffffffff) {
         ...
         snprintf (buf, sizeof (buf), "<< About to call codext_init with shell: 0x%08x (offset of %d not yet applied), shell_len: %u, eip: 0x%08x\n", (uint) shell, curr_offset, len, eip);
         #ifdef SYMB
         s2e_message (buf);
         ...
         printf ("<< Calling shell: 0x%08x (adjusted by offset of %u), of len: %u with eip: 0x%08x (%u) and syscall: %u\n", (uint) shell, curr_offset, len, eip, eip - (maddr_h) shell, syscall);
			/* technically/theoretically, this should be {push shell; ret} as that would mimic a real world exploit */
         shell ();
...
void normalizeShellcode (struct shellcode* s, uint f) {
   ...
   snprintf (buf, sizeof (buf), "<< About to call codext_init with shell: 0x%08x (offset of %u not yet applied), shell_len: %u\n", lower_bounds, f, s->len);
   s2e_message (buf);
   
   s2e_codext_init (lower_bounds, s->len, EIP_UNKNOWN, SYSC_UNKNOWN);
            ...
            switch (argv[i][1]) {
               ...
               case 'i' :
                  ...
                  if (isThisADasosfDump (test_inp, DASOSFDUMP_PREAMBLE_LEN) ) {
                     ...
                     struct stat st;
                     stat (argv[i], &st);
                     o->shellcode.len = st.st_size;
                     //readFileToShell (argv[i], &(o->shellcode) );
                     FILE* SHELL;
                     if ((SHELL = fopen (argv[i], "r") ) == NULL) {
                        fprintf (stderr, "DASOSF: Can't open raw shell file: %s\n", strerror (errno) );
                        return;
                     }
                     
                     // read from file, up to size of shell
                     o->shell = malloc (sizeof (uint8_t) * o->shellcode.len); // TODO free at end
                     o->shellcode.len = fread (o->shell, 1, o->shellcode.len, SHELL);
                     // normalize read to units that shell->shell uses
                     o->shellcode.len = o->shellcode.len / sizeof (byte_t);
                     fclose (SHELL);
