...
bool DasosPreproc::isExecTraceSubset (Exec_Trace needle, Exec_Trace haystack) { 
   if (haystack.insns.size () == 0) {
      return false;
   }
   // for each offset in haystack except for a suffix of length less than needle
   for (unsigned i = 0; i < (haystack.insns.size () - needle.insns.size () + 1); i++) {
      unsigned j = 0;
...
void DasosPreproc::onTransOOBInsns (S2EExecutionState* state, TranslationBlock* tb, uint64_t pc, uint64_t len, uint64_t nextpc) {
   ...
   if (plgState->within_range) {
      ...
      s2e()->getWarningsStream (state) << "@0x" << std::noshowbase << std::setw(8) << std::setfill('0') << std::hex << pc << ", left buffer range after " << std::dec << plgState->execed_insns << " IoB exec'ed insns; last IoB insn @0x" << std::noshowbase << std::setw(8) << std::setfill('0') << std::hex << plgState->exec_trace.insns.back().addr + cfg.base_addr << std::dec << ", disasm in debug.\n";
...
/*void DasosPreproc::handleIfFPU2 (S2EExecutionState* state, exec_instance e) {
   DECLARE_PLUGINSTATE (DasosPreprocState, state);
   // if fnstenv D9 / 6 ; possibly also fstenv 9B D9 / 6 (and fsav fnsav/fxsav)
   // look at i386-translate.c:5692-ish
   // this insn should write a FPU exception struct to the given address, but the struct isn't handled correctly (the last fpu pc not set upon any fpu insn
...
void DasosPreproc::onSyscall (S2EExecutionState* state, uint64_t pc, uint32_t sysc_number) {
   ...
   // TODO if success make a semantic analysis given the sysc_number, look at the parameters and see if they match up to prototypes
...
void DasosPreproc::onFragment (S2EExecutionState* state, uint64_t pid, uint64_t pc, uint32_t sysc_number, unsigned len) {
   DECLARE_PLUGINSTATE (DasosPreprocState, state);
   s2e()->getDebugStream() << ">> onFragment syscall found. Syscall number 0x" << std::hex << sysc_number << " at addr 0x" << std::noshowbase << std::setw(8) << std::setfill('0') << pc << " offset from base: " << std::dec << (pc - cfg.base_addr) << " (should be EIP-" << std::dec << len << ") within pid: " << pid << " number of exec'ed instructions: " << plgState->exec_trace.insns.size () << ". This is fragment #" << cfg.fragments.size () + 1 << "\n";
   ...
   // else
   terminateStateEarly_wrap (state, std::string ("A syscall fragment found, terminating state"), true);
   return;
} // end fn onFragment
...
void DasosPreproc::onFini (S2EExecutionState* state) {
   ...
   if (cfg.fragments.size () > 0) {
      ...
      for (unsigned i = 0; i < cfg.fragments.size (); i++) {
         ...
         /*if (odens_max < f->overlay_density) {
            odens_max = f->overlay_density;
            odens_max_idx = i;
         }
         if (adens_max < f->avg_density) {
            adens_max = f->avg_density;
            adens_max_idx = i;
         }*/
         bool exists = false;
         for (unsigned j = 0; j < eips.size (); j++) {
            if (f->eip_addr == eips[j]) {
               exists = true;
            }
         }
         if (!exists) {
            eips.push_back (f->eip_addr);
         }
...
/*Chunk DasosPreproc::createChunk (struct Fragment_t f) {
   Chunk c;
   c.push_back (f);
   return c;
} // end fn createChunk */
...
void DasosPreproc::createCodeChunks (unsigned match_frag) {
   //Chunk match = createChunk ((struct Fragment_t) cfg.successes[match_frag]);
...
void DasosPreproc::printChunk (Chunk c) {
   ...
   for (unsigned i = 0; i < c.size (); i++) {
      s2e()->getDebugStream() <<  ">> Chunk fragment " << i << " is " << (c[i].is_success ? "success":"fragment") << " starts at " << c[i].exec_trace.insns[0].addr + cfg.base_addr << " and ends at " << c[i].exec_trace.insns.back().addr + cfg.base_addr << "\n";
   }
   return;
