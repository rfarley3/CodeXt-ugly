...
struct insn_instance {
   ...
   uint64_t jmp_pc;        // if it's a jmp insn and the next_pc is sequential, find its jump addr
...
public:
   ...
   void onTranslateBlockEnd (ExecutionSignal *signal, S2EExecutionState *state, TranslationBlock *tb, uint64_t pc, bool, uint64_t);
...
private:
   ...
   sigc::connection oTBE_connection;
   bool oTBE_connected;
   ...
   unsigned tot_killable_insns;
   uint32_t offset;
   uint64_t kernel_insns;
   uint64_t pc_of_next_insn_from_last_IoB;
   uint64_t pc_of_next_insn;
   bool     expecting_jmp_OOB;
