...
void DasosPreproc::onTransIOBInsns (S2EExecutionState* state, uint64_t pc, TranslationBlock *tb) {
   ...
   /*if (tb->write_len > 0) {
      s2e()->getDebugStream() << ">> !! Write observed, " << tb->write_len << "B, @0x" << std::noshowbase << std::setw(8) << std::setfill('0') << std::hex << tb->write_addr << "\n";
   }*/
...
void DasosPreproc::onTranslateInstructionEnd (ExecutionSignal *signal, S2EExecutionState* state, TranslationBlock *tb, uint64_t pc, uint64_t len) {
   DECLARE_PLUGINSTATE (DasosPreprocState, state);
   if (isInShell (pc) ) {
      s2e()->getDebugStream() << " >> oTIE pc: 0x" << std::hex << std::noshowbase << std::setw(8) << std::setfill('0') << pc << ":0x" << std::noshowbase << std::setw(2) << std::setfill('0') << pc - cfg.base_addr << " len: " << std::dec << len << "\n";
   }
...
void DasosPreproc::onTranslateBlockEnd (ExecutionSignal *signal, S2EExecutionState *state, TranslationBlock *tb, uint64_t pc, bool is_target_valid, uint64_t target_pc) {
   ...
   if (isInShell (pc) && is_target_valid) {
