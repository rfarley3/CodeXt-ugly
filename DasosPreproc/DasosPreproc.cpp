...
void DasosPreproc::initialize() {
   ...
   s2e()->getDebugStream () << ">> luaLoad: maxKernelInsn " << cfg.max_kernel_insn << '\n'; 
	cfg.max_killable_insn = s2e()->getConfig()->getInt(getConfigKey() + ".maxKillableInsn", flush_tb_on_change);
   s2e()->getDebugStream () << ">> luaLoad: maxKillableInsn " << cfg.max_killable_insn << '\n'; 
	...
	for (uint32_t i = 0; i < symb_data.size (); i++) {
		struct symbVar_t symb_var;
		...
		symb_var.len  = atoi (symb_data[i].c_str() ); i++; // length in num bytes of symbolic variable
		symb_var.when = atoi (symb_data[i].c_str() );      // at which insn to make symbolic
		symb_var.marked = 0;
		symb_var.exprs.reserve (symb_var.len);
		cfg.symb_vars.push_back (symb_var);
   	s2e()->getDebugStream () << ">> luaLoad: symbVars[" << int (i/4) << "]: " << symb_var.name << " at " << symb_var.addr << " for " << symb_var.len << "B after insn " << symb_var.when << '\n'; 
	}
	...
	for (uint32_t i = 0; i < symb_data.size (); i++) {
		struct symbVar_t symb_var;
		symb_var.name = symb_data[i]; i++;                 // name of var to watch, not used by program, only useful to user
		...
		symb_var.when = atoi (symb_data[i].c_str() );      // at which insn to start monitoring
		symb_var.marked = 0;
		symb_var.exprs.reserve (symb_var.len);
		cfg.monitor_vars.push_back (symb_var);
   	s2e()->getDebugStream () << ">> luaLoad: monitorVars[" << int (i/4) << "]: " << symb_var.name << " at " << symb_var.addr << " for " << symb_var.len << "B after insn " << symb_var.when << '\n'; 
	}
   ...
   s2e()->getDebugStream () << ">> luaLoad: multisysc " << (cfg.allow_multi_sysc ? "true" : "false") << '\n'; 
	...
	cfg.enableMultipath = s2e()->getConfig()->getBool (getConfigKey() + ".multiPath", false); 
   s2e()->getDebugStream () << ">> luaLoad: multipath " << (cfg.enableMultipath ? "true" : "false") << '\n'; 
   
   // Set a hook for the custom insns
   //custom_instruction_connection = new sigc::connection (s2e()->getCorePlugin()->onCustomInstruction.connect (sigc::mem_fun (*this, &DasosPreproc::onCustomInstruction) ) );
   custom_instruction_connection = s2e()->getCorePlugin()->onCustomInstruction.connect (sigc::mem_fun (*this, &DasosPreproc::onCustomInstruction) );

   return;
} // end fn initialize


std::string DasosPreproc::hex (unsigned int bytes, klee::ref<klee::ConstantExpr> const_val) {
		uint64 val = (uint64_t) cast<klee::ConstantExpr>(const_val)->getZExtValue (64);
		return hex (bytes, val);
} // end fn hex klee


//std::ostringstream DasosPreproc::hex (unsigned int bytes, uint64_t val) {
std::string DasosPreproc::hex (unsigned int bytes, uint64_t val, bool showbase) {
	uint64_t mask;
	switch (bytes) {
		case 1:
			mask = 0xff;
...
uint32_t DasosPreproc::getTopOfStack (S2EExecutionState* state) {
	uint64_t esp_addr = 0;
...
bool DasosPreproc::readMemory (S2EExecutionState* state, uint64_t address, void* buf, uint64_t size) {
...
klee::ref<klee::Expr> DasosPreproc::readMemoryByte (S2EExecutionState* state, uint64_t addr) {
	klee::ref<klee::Expr> v = state->readMemory8 (addr, state->VirtualAddress);
   if (v.isNull () ) {
		s2e()->getDebugStream () << "!! ERROR: readMemoryByte: null expression\n";
		return klee::ConstantExpr::create (0, klee::Expr::Int8);
   }
	return v;
} // end fn readMemoryByte
...
void DasosPreproc::onCustomInstruction (S2EExecutionState* state, uint64_t opcode) {
   ...
   switch (op) {
      case 1:
         ...
         if (!ok) {
            s2e()->getWarningsStream (state) << "!! ERROR: symbolic argument was passed to s2e_op in DasosPreproc loadmodule" << '\n';
            return;
         }
      ...
      case 2:
         ...
         if (!ok) s2e()->getWarningsStream (state) << "!! ERROR: bad argument was passed to s2e_op: start " << start << " in DasosPreproc start fuzzing" << '\n';
         ok &= state->readCpuRegisterConcrete(CPU_OFFSET(regs[R_ECX]), &(end), 4);
         end = end & 0xffffffff;
         if (!ok) s2e()->getWarningsStream (state) << "!! ERROR: bad argument was passed to s2e_op: end " << end << " in DasosPreproc start fuzzing" << '\n';

         if (!ok) return;
         
         if (start > end) {
            s2e()->getWarningsStream (state) << "!! ERROR: start (" << start << ") > end (" << end << ") is invalid range in DasosPreproc start fuzzing" << '\n';
            return;
         }
      ...
      case 4:
         ...
         if (!ok) {
            s2e()->getWarningsStream (state) << "!! ERROR: bad argument was passed to s2e_op: start " << std::dec << value << " in DasosPreproc start fuzzing" << '\n';
            return;
         }
...
void DasosPreproc::onActivateModule (S2EExecutionState* state) {
   if (!cfg.eip_valid) {
      ...
      s2e()->getWarningsStream (state) << "!! ERROR: EIP " << hex (4, cfg.eip_addr) << " given to DasosPreproc is not within range " << hex (4, cfg.base_addr) << "-" << hex (4, cfg.end_addr) << '\n';
   ...
   s2e()->getDebugStream () << ">> Recv'ed custom insn for a DasosPreproc memory segment within pid " << cfg.proc_id << ", addr range: " << hex (4, cfg.base_addr) << "-" << hex (4, cfg.end_addr) << " with eip: " << hex (4, cfg.eip_addr) << " buffer length: " << cfg.byte_len << " and syscall number: " << cfg.sysc << '\n';
   ...
   plgState->oRA_connection =  plg->onTranslateRegisterAccessEnd.connect (sigc::mem_fun (*this, &DasosPreproc::onTranslateRegisterAccess) );
   ...
   // onTranslateInstructionEnd_RJF is a modified S2E core function that returns the length of the src bytes of instruction translated
   plgState->oSC_connection = plg->onSilentConcretize.connect (sigc::mem_fun (*this, &DasosPreproc::onSilentConcretize) );
...
void DasosPreproc::onTranslateBlockStart (ExecutionSignal* signal, S2EExecutionState* state, TranslationBlock* tb, uint64_t pc) {
   if (isInShell (state->getPc () ) ) {
      DECLARE_PLUGINSTATE (DasosPreprocState, state);
      s2e()->getDebugStream () << " >> oTBS pc: " << hex (4, pc) << " tb_seq_num: " << std::dec << (plgState->tb_seq_num + 1) << '\n'; 
      plgState->tb_seq_num++;
   }
   return;
...
void DasosPreproc::onTranslateInstructionEnd (ExecutionSignal* signal, S2EExecutionState* state, TranslationBlock* tb, uint64_t pc, uint64_t nextpc) {
   ...
   if (isInShell (pc) ) {
      ...
      s2e()->getDebugStream () << " >> oTIE pc: " << hex (4, pc) << ":" << hex (4, pc - cfg.base_addr) << " nextpc: " << hex (4, nextpc) << " len: " << std::dec << len << '\n';
   }
   ...
   if (plgState->trans_trace.insns.size () != 0 && pc == plgState->trans_trace.insns.back().addr ) {
      s2e()->getDebugStream () << "!!* pc == plgState->pcs.back @ 0x" << std::hex << std::noshowbase << std::setw(8) << std::setfill('0') << pc << std::dec << " of len " << tb->size << "B, the 1st is 0x" << std::noshowbase << std::setw(8) << std::setfill('0') << ((unsigned) (tb->tc_ptr)[0] & 0x000000ff) << '\n';
      return;
   }*/
   ...
   if (plgState->in_range_insns == 0) {
		s2e()->getDebugStream () << "First Insn dumpX86State:\n";
		state->dumpX86State (s2e()->getDebugStream () );
		s2e()->getDebugStream () << "Top of stack: " << hex (4, getTopOfStack (state) ) << '\n';
	}
...
const char * const DasosPreproc::X86_REG_NAMES[] = {"EAX", "ECX", "EDX", "EBX", "ESP", "EBP", "ESI", "EDI", "UNK"};

void DasosPreproc::addRegWrite (uint64_t pc, uint8_t reg, uint64_t offset, uint64_t seq_num, Reg_Write_Trace &trace) {
	//std::string X86_REG_NAMES[] = {"EAX", "ECX", "EDX", "EBX", "ESP", "EBP", "ESI", "EDI", "UNK"};
	// TODO put boundary/segfault assert here, st reg < size of reg_names
	if (reg > 7) {
		reg = 8;
	}
   ...
   reg_write.other_pc = pc;       // if it's a data event, then this is the writer address
   reg_write.in_range = false;    // whether it is in the range (ie if the bytes were recorded into the code_map/snapshot
   reg_write.valid = true;        // whether it is an insn worth using in comparisons (ie is a repeat)
	trace.push_back (reg_write);
	s2e()->getDebugStream () << " >> oTRA: pc " << hex (4, pc) << " write to " << X86_REG_NAMES[reg] << '\n';
	return;
} // end fn addRegWrite
...
void DasosPreproc::onTranslateRegisterAccess (ExecutionSignal* signal, S2EExecutionState* state, TranslationBlock* tb, uint64_t pc, uint64_t read_mask, uint64_t write_mask, bool isMemoryAccess) {
   ...
   DECLARE_PLUGINSTATE (DasosPreprocState, state);
	bool is_write = false;
	if ( (write_mask & (1 << R_ESP) ) ) {
		is_write = true;
		addRegWrite (pc, R_ESP, CPU_OFFSET (regs[R_ESP]), plgState->seq_num, plgState->reg_trace);

   }
	if ( (write_mask & (1 << R_EAX) ) ) {
		is_write = true;
		addRegWrite (pc, R_EAX, CPU_OFFSET (regs[R_EAX]), plgState->seq_num, plgState->reg_trace);
	}
	if ( (write_mask & (1 << R_EBX) ) ) {
		is_write = true;
		addRegWrite (pc, R_EBX, CPU_OFFSET (regs[R_EBX]), plgState->seq_num, plgState->reg_trace);
	}
	if ( (write_mask & (1 << R_ECX) ) ) {
		is_write = true;
		addRegWrite (pc, R_ECX, CPU_OFFSET (regs[R_ECX]), plgState->seq_num, plgState->reg_trace);
	}
	if ( (write_mask & (1 << R_EDX) ) ) {
		is_write = true;
		addRegWrite (pc, R_EDX, CPU_OFFSET (regs[R_EDX]), plgState->seq_num, plgState->reg_trace);
	}
	if ( (write_mask & (1 << R_ESI) ) ) {
		is_write = true;
		addRegWrite (pc, R_ESI, CPU_OFFSET (regs[R_ESI]), plgState->seq_num, plgState->reg_trace);
	}
	if ( (write_mask & (1 << R_EDI) ) ) {
		is_write = true;
		addRegWrite (pc, R_EDI, CPU_OFFSET (regs[R_EDI]), plgState->seq_num, plgState->reg_trace);
	}
	if ( (write_mask & (1 << R_EBP) ) ) {
		is_write = true;
		addRegWrite (pc, R_EBP, CPU_OFFSET (regs[R_EBP]), plgState->seq_num, plgState->reg_trace);
	}
	
	if (is_write) {
		//s2e()->getDebugStream () << " >> oTRA pc: " << hex (4, pc) << " write(s). read_mask: " << hex (8, read_mask) << " write_mask: " << hex (8, write_mask) << " isMemAccess? " << isMemoryAccess << "\n";
	   signal->connect (sigc::mem_fun (*this, &DasosPreproc::onRegisterAccess) );
	}
	return;
} // end fn onTranslateRegisterAccess


void DasosPreproc::onRegisterAccess (S2EExecutionState* state, uint64_t pc) {
   DECLARE_PLUGINSTATE (DasosPreprocState, state);
	//std::string X86_REG_NAMES[] = {"EAX", "ECX", "EDX", "EBX", "ESP", "EBP", "ESI", "EDI", "UNK"};
	//char const* X86_REG_NAMES[] = {"EAX", "ECX", "EDX", "EBX", "ESP", "EBP", "ESI", "EDI", "UNK"};
	//uint8_t reg = 8;
	for (int i = plgState->reg_trace.size () - 1; i >= 0; i--) {
		if (plgState->reg_trace[i].other_pc == pc) {
			// get bytes within register...
			// reg_trace[i].bytes[j] = readRegister
   		//plgState->write_trace.writes.push_back ((Data_Instance) reg_trace[i]);
      	...
      	data.bytes[i] = byte;
			...
			// if pc is in WasConcretized Queue, then mark all writes with original symbolic label; do same for oDMA
			s2e()->getDebugStream () << " >> oRA pc: " << hex (4, pc) << " write to " << X86_REG_NAMES[plgState->reg_trace[i].is_register] << '\n';
		}
	}
	// TODO clean up reg_trace
	return;
...
// this should be very similar to taintMem (try to share the actual writer)
void DasosPreproc::taintReg (S2EExecutionState* state, uint8_t reg, uint64_t offset, uint64_t concretized_addr) {
	// pull out label (or make a new accessor and initial store mechanism)
	klee::ref<klee::Expr> tainted_expr = getOriginalTaint (concretized_addr);
	klee::ref<klee::Expr> taint_label = extractLabel (state, tainted_expr); // keep in mind that it could already have multiple labels
	...
	for (unsigned i = 0; i < 4; i++) { // 32b, so regs are 4B
      ...
      s2e()->getDebugStream () << " >> taintReg byte_expr: " << byte_expr << '\n';
      ...
      s2e()->getDebugStream () << " >> taintReg tainted_byte: " << tained_byte << '\n';
...
// etc...
klee::ref<klee::Expr> DasosPreproc::extractLabel (S2EExecutionState* state, klee::ref<klee::Expr> tainted_expr) {
   return state->createSymbolicValue (klee::Expr::Int8, "temporaryExtractedLabel");
} // end fn extractLabel

					
void DasosPreproc::onExecuteInsn (S2EExecutionState* state, uint64_t pc) {
   if (!isInShell (pc) ) {
      s2e()->getDebugStream () << " >> oEI OOB pc: " << hex (4, pc) << '\n';
      return;
   }
   s2e()->getDebugStream () << " >> oEI pc: " << hex (4, pc) << '\n';
   ...
   if (e.len == 0 || e.bytes.size () != e.len) {
      s2e()->getDebugStream () << "!! ERROR: oEI failed to find a translation, pc: " << hex (4, pc) << '\n';
   ...
   plgState->execed_insns++;
	...
	//bool had_been_concretized = false;
	...
	for (int i = plgState->concretize_trace.size () - 1; i >= 0; i--) {
		uint64_t concretized_addr = plgState->concretize_trace[i];
		if (concretized_addr >= pc && concretized_addr <= (pc + e.len) ) {
	      s2e()->getDebugStream () << " >> oEI pc: " << hex (4, pc) << " had been silently concretized at addr: " << hex (4, concretized_addr) << '\n';
         plgState->concretize_trace.erase (plgState->concretize_trace.begin () + i);
			//had_been_concretized = true;
			// see if something overwrote that byte (it's not longer symbolic)
			if (readMemoryByte (state, concretized_addr) != getOriginalTaint (concretized_addr) ) {
				// something overwrote that byte after the original taint was restored...
				// there is a chance that the insn was overwritten concretely (by an insn other than itself), if so then label should be not be restored to insn
     		 	s2e()->getDebugStream () << "!! WARNING: oEI post-taint-restore , pc: " << hex (4, pc) << " addr: " << hex (4, concretized_addr) <<'\n';
			}
			for (int i = plgState->reg_trace.size () - 1; i >= 0; i--) {
				// if the reg write was done by this insns
				if (plgState->reg_trace[i].other_pc == pc) {
			      s2e()->getDebugStream () << " >> oEI pc: " << hex (4, pc) << " wrote to reg: " << X86_REG_NAMES[plgState->reg_trace[i].is_register] << '\n';
					taintReg (state, plgState->reg_trace[i].is_register, plgState->reg_trace[i].addr, concretized_addr);
		         plgState->reg_trace.erase (plgState->reg_trace.begin () + i);
					// do we need to trigger a retranslation for further instructions?
					plgState->oEI_retranslate = pc;
					// need to purge the reg_trace and concretize_trace since all pending translations are tossed out.
				}
			}
			// for each DMA taint the written bytes
		}
	}
	...
	for (unsigned i = 0; i < e.len; i++) {
   	if (!readMemory (state, pc + i, &(insn_raw[i]), 1) ) {
		...
		// else is symbolic, so try to make a concrete example of the symb expr
         ...
         s2e()->getDebugStream () << " >> WARNING: oEI bytes in mem (executed) do not match bytes when translated, pc: " << hex (4, pc) << " trans: ";
   ...
   if (plgState->oEI_retranslate == pc) {
      s2e()->getDebugStream () << " >> DEBUG: oEI retranslate triggered at pc " << hex (4, pc) << '\n';
      plgState->oEI_retranslate = 0;

		// TODO			// need to purge the reg_trace and concretize_trace since all pending translations are tossed out.
      ...
      //os << "EAX=0x" << std::hex << readCpuRegister(offsetof(CPUState, regs[R_EAX]), klee::Expr::Int32) << '\n';
      // ref<Expr> S2EExecutionState::readCpuRegister(unsigned offset, Expr::Width width) const
      symb_val = state->readCpuRegister(offsetof(CPUState, regs[R_EAX]), klee::Expr::Int32);
      if (symb_val.isNull() || !isa<klee::ConstantExpr>(symb_val)) {
         s2e()->getMessagesStream (state) << "Reg EAX is symbolic, expr: " << symb_val << '\n';
      }
      else {
         conc_val = (uint8_t)cast<klee::ConstantExpr>(symb_val)->getZExtValue(32);
         s2e()->getMessagesStream (state) << "Reg EAX is concrete, val:  0x" << std::hex << std::setw(2) << std::setfill('0') << (unsigned int) conc_val << "; expr: " << symb_val << '\n';
      }*/
      
	if (1) {  // mark specific variables as symbolic if trigger has occured, and check if specific variables are concrete or symbolic
      uint8_t conc_val;
      klee::ref<klee::Expr> symb_val;
		for (uint32_t i = 0; i < cfg.symb_vars.size (); i++) {
			s2e()->getMessagesStream (state) << cfg.symb_vars[i].name << " (" << hex (4, cfg.base_addr + cfg.symb_vars[i].addr) << ") ";
	      if (isSymb_extended (state, cfg.base_addr + cfg.symb_vars[i].addr, conc_val, symb_val) ) {
	         s2e()->getMessagesStream (state) << "is symbolic, 1st byte's expr: " << symb_val << '\n';
	         //printSymb (state, key_addr);
	      }
	      else {
	         s2e()->getMessagesStream (state) << "is concrete, 1st byte's val:  " << hex (1, conc_val) << "; expr: " << symb_val << '\n';
				if (!cfg.symb_vars[i].marked && plgState->execed_insns >= cfg.symb_vars[i].when) {
		         markSymb (state, cfg.symb_vars[i]);
					cfg.symb_vars[i].marked = 1;
					s2e()->getMessagesStream (state) << cfg.symb_vars[i].name << " (" << hex (4, cfg.base_addr + cfg.symb_vars[i].addr) << ") ";
				   symb_val = state->readMemory8 (cfg.base_addr + cfg.symb_vars[i].addr, state->VirtualAddress);
					s2e()->getMessagesStream (state) << "marked symbolic, 1st byte's expr: " << symb_val << '\n';
					//s2e()->getMessagesStream (state) << "simplified, expr: " << state->constraints.simplifyExpr (symb_val) << ' ';
				   klee::ref<klee::ConstantExpr> const_val;
				   if (!s2e()->getExecutor()->getSolver()->getValue (klee::Query (state->constraints, symb_val), const_val) ) {
						// failure
				   } //(state, symb_val, const_val);
					s2e()->getMessagesStream (state) << "asConstant, 1st byte's expr: " << const_val << ":" << hex (1, const_val) << '\n';
					//s2e()->getMessagesStream (state) << "constraints.size: " << state->constraints.size () << '\n';
					//std::pair<klee::ref<klee::Expr>, klee::ref<klee::Expr> > range = s2e()->getExecutor()->getSolver()->getRange (klee::Query (state->constraints, symb_val) );
					//                expr = s2e()->getExecutor()->toConstantSilent(*state, expr);
					//state, cfg.base_addr + cfg.symb_vars[i].addr); //Query (state->constraints, symb_val) );
					//s2e()->getMessagesStream (state) << "range low: " << range.first << " high: " << range.second << '\n';
				}
	      }
		}
		for (uint32_t i = 0; i < cfg.monitor_vars.size (); i++) {
			if (plgState->execed_insns >= cfg.monitor_vars[i].when) {
				uint32_t monitor_addr = 0;
				bool monitor_printed = 0;
				if (cfg.monitor_vars[i].name[0] == '_') { // register
				   klee::ref<klee::ConstantExpr> const_val;
					// if the address is symbolic
					klee::ref<klee::Expr> addr_symb = klee::ConstantExpr::create (0, klee::Expr::Int32);
		         if (!(state->readCpuRegisterConcrete (cfg.monitor_vars[i].addr, &(monitor_addr), sizeof (uint32_t) ) ) ) {
						addr_symb = state->readCpuRegister (cfg.monitor_vars[i].addr, klee::Expr::Int32);
				   	if (!s2e()->getExecutor()->getSolver()->getValue (klee::Query (state->constraints, addr_symb), const_val) ) {
							// failure
				   	} //(state, symb_val, const_val);
						monitor_addr = (uint32_t) cast<klee::ConstantExpr>(const_val)->getZExtValue (32);
		         }
					s2e()->getMessagesStream (state) << "Word value inside register " << cfg.monitor_vars[i].name << " is: " << hex (4, monitor_addr) << '\n';
					s2e()->getMessagesStream (state) << "Symb expr: " << addr_symb << '\n';
					// DEBUG, print the word at this location
					symb_val = state->readMemory (monitor_addr, klee::Expr::Int32);//state->VirtualAddress);
				   if (!s2e()->getExecutor()->getSolver()->getValue (klee::Query (state->constraints, symb_val), const_val) ) {
				   	// failure
					}
				...
				// else it's not a register, so it's a memory address
					...
					monitor_addr = cfg.base_addr + cfg.monitor_vars[i].addr;
				}
				// for each byte in monitor var
				for (uint32_t j = 0; !monitor_printed && j < cfg.monitor_vars[i].len; j++) {
					s2e()->getMessagesStream (state) << cfg.monitor_vars[i].name << " (" << hex (4, monitor_addr + j) << ") ";
		      	if (isSymb_extended (state, monitor_addr + j, conc_val, symb_val) ) {
		         	s2e()->getMessagesStream (state) << "is symbolic, expr: " << symb_val << ' ';
						//s2e()->getMessagesStream (state) << "is symbolic, simplified expr: " << state->constraints.simplifyExpr (symb_val) << ' ';
						// toConstant takes an expression and returns a constant value for it
						//symb_val = s2e()->getExecutor()->toConstant (*state, symb_val, "test concretize"); 
						//s2e()->getMessagesStream (state) << "toConstant, expr: " << symb_val << ' ';
						//s2e()->getMessagesStream (state) << "constraints.size: " << state->constraints.size () << '\n';
					   klee::ref<klee::ConstantExpr> const_val;
					   if (!s2e()->getExecutor()->getSolver()->getValue (klee::Query (state->constraints, symb_val), const_val) ) {
					   	// failure
					   } //(state, symb_val, const_val);
						s2e()->getMessagesStream (state) << "asConstant, expr: " << const_val << ":" << hex (1, const_val) << '\n';
					
		      	}
		      	else {
		         	s2e()->getMessagesStream (state) << "is concrete, val: " << hex (1, conc_val) << "; expr: " << symb_val << '\n';
		      	}
				}
			}
		}
...
void DasosPreproc::markSymb (S2EExecutionState* state, struct symbVar_t svar) {
	...
	uint64_t symbVar_idx = 0;
	for (uint64_t i = 0; i < cfg.symb_vars.size (); i++) {
		if (svar.addr == cfg.symb_vars[i].addr) {
			symbVar_idx = i;
			//if (address >= cfg.symb_vars[i].addr && address < (cfg.symb_vars[i].addr + cfg.symb_vars[i].len) ) {
			//cfg.symb_vars[i].exprs.pushback (symb);
		}
	}
   s2e()->getDebugStream () << ">> markSymb: using symbVar_idx: " << symbVar_idx << '\n';
	cfg.symb_vars[symbVar_idx].exprs.resize (svar.len);
   s2e()->getDebugStream () << ">> markSymb: length: " << svar.len << '\n';
	
	for (uint32_t i = 0; i < svar.len; i++) {
		//std::string idx;
		...
		klee::ref<klee::Expr> taint = markSymbTagged (state, cfg.base_addr + svar.addr + i, name);
	   s2e()->getDebugStream () << ">> markSymb: inserting at [" << i << "]: " << taint << '\n';
		cfg.symb_vars[symbVar_idx].exprs[i] = taint;
...
void DasosPreproc::markSymbConstraint (S2EExecutionState* state, uint32_t address, std::string nameStr) {
   char buf; // consider typing as uint8_t for consistency
   // test if isSymb
   if (!state->readMemoryConcrete (address, &buf, 1) ) { //isSymb (state, address) ) {
      s2e()->getMessagesStream (state) << "Skipping markSymb request, byte already symbolic\n";
   ...
   klee::ref<klee::Expr> symb = state->createSymbolicValue (klee::Expr::Int8, nameStr);
	// the following doesn't work bc addConstraint is protected. could call it round'about via toConstant
   //klee::ref<klee::Expr> symb_const = klee::ConstantExpr::create ((uint8_t) buf, klee::Expr::Int8);
 	//s2e()->getExecutor()->addConstraint (*state, klee::EqExpr::create (symb, symb_const) );
	// but toConstant doesn't use the conrete value at all, or the address
	//symb_val = s2e()->getExecutor()->toConstant (*state, symb, "test concretize"); 
   ...
   return;
} // end fn markSymbConstraint


klee::ref<klee::Expr> DasosPreproc::markSymbTagged (S2EExecutionState* state, uint32_t address, std::string nameStr) {
   char buf; // consider typing as uint8_t for consistency
   if (!state->readMemoryConcrete (address, &buf, 1) ) { //isSymb (state, address) ) {
   ...
   klee::ref<klee::Expr> symb = klee::AddExpr::create (symb_const, symb_taint);
	...
	// isn't going to work when buf is 00
	//klee::ref<klee::Expr> e_min = klee::ConstantExpr::create ((uint8_t) buf - 1, klee::Expr::Int8); 
	//klee::ref<klee::Expr> e_max = klee::ConstantExpr::create ((uint8_t) buf + 1, klee::Expr::Int8);
	//klee::ref<klee::Expr> constraint_min = klee::UltExpr::create (e_min, symb);
	//klee::ref<klee::Expr> constraint_max = klee::UgtExpr::create (e_max, symb);
	//klee::ref<klee::Expr> e_min = klee::ConstantExpr::create ((uint8_t) buf - , klee::Expr::Int8); 
	//klee::ref<klee::Expr> e_max = klee::ConstantExpr::create ((uint8_t) buf + 1, klee::Expr::Int8);
	klee::ref<klee::Expr> constraint_min = klee::UleExpr::create (symb_const, symb);
	klee::ref<klee::Expr> constraint_max = klee::UgeExpr::create (symb_const, symb);
	state->addConstraint (constraint_min);
...
void DasosPreproc::marksSymbAddConstExpr (S2EExecutionState* state, uint32_t address, std::string nameStr) {
   char buf; // consider typing as uint8_t for consistency
   //klee::ref<klee::Expr> symb_val;
...
void DasosPreproc::onSyscall (S2EExecutionState* state, uint64_t pc, uint32_t sysc_number) {
   ...
   if (plgState->exec_trace.insns.size() < cfg.min_exec_insns) { 
	...
	for (uint64_t i = 0; i < plgState->exec_trace.insns.size (); i++) {
		execBytes += plgState->exec_trace.insns[i].len;
	}
   if (execBytes < cfg.min_exec_bytes) { 
...
void DasosPreproc::onException (S2EExecutionState* state, unsigned exception_idx, uint64_t pc) {
   if (isInShell (pc) ) {
      s2e()->getDebugStream () << " >> oExc pc: " << hex (4, pc) << " exception_idx: " << std::dec << exception_idx << "(" << hex (4, exception_idx) << ")\n";
      //state->dumpX86State(s2e()->getDebugStream () );
      ...
      if (exception_idx == 0x80) {
         ...
         s2e()->getDebugStream () << " >> oExc INT 0x80 pc: " << hex (4, pc) << " syscall_num: " << std::dec << int_num << "(" << hex (4, int_num) << ")\n";
         //state->dumpX86State(s2e()->getDebugStream () );
...
void DasosPreproc::onSilentConcretize (S2EExecutionState* state, uint64_t concretized_byte_addr, uint8_t concrete_val, const char* reason) {
   DECLARE_PLUGINSTATE (DasosPreprocState, state);
   if (!plgState->has_entered_range) {
		return;
	}
	...
	klee::ref<klee::Expr> taint = getOriginalTaint (concretized_byte_addr);
...
klee::ref<klee::Expr> DasosPreproc::getOriginalTaint (uint64_t pc) {
   ...
   for (unsigned i = 0; i < d.writes.size (); i++) {
      if (d.writes[i].in_range) {
         ...
         for (unsigned j = 0; j < d.writes[i].bytes.size (); j++) {
            ...
            else if (m.size () > 1 && (d.writes[i].seq_num - last_write_seq_num) >= cfg.cluster_writes_by) { // 10
               diff_cluster = true;
            }
...
void DasosPreproc::fuzzFork2 (S2EExecutionState* state, unsigned int value) {
   /** Emulate fork via WindowsApi forkRange Code */
   DECLARE_PLUGINSTATE (DasosPreprocState, state);
...
/*
fuzzFork2 calls S2EExecutor::StatePair S2EExecutor::fork(ExecutionState &current,
