...
typedef struct Success_t Success;
...
public:
   ...
   */
   ...
   void terminateStateEarly_wrap (S2EExecutionState* state, std::string msg, bool success);
   
   
   // Helper functions
   bool isInsnImpossibleFirst (uint8_t* raw_insn, unsigned raw_insn_len);
   bool areInsnInstancesEqual (exec_instance i1, exec_instance i2); //, Mem_map m);
   //bool isInsnRepeat (trans_instance i2, trans_instance i1, Mem_map m);
   //unsigned findNextInRange (Trans_Trace t, unsigned i);
   unsigned findNextValid (Exec_Trace t, unsigned i);
   bool hasBeenTranslated (S2EExecutionState* state, uint64_t addr, unsigned len);
   //uint64_t getSeqNum (S2EExecutionState* state, uint64_t writer);
   //bool hasByteChanged (uint8_t byte, Mem_map m, uint64_t addr);
   //void validateInsn (S2EExecutionState* state, trans_instance insn, uint8_t* raw);
   //void invalidateInsn (S2EExecutionState* state, unsigned idx, trans_instance cause);
   ...
   bool isInsnTraceUnique (Exec_Trace t, std::vector<Success> s); //, Mem_map m);
   bool isInsnTraceSubset (Exec_Trace needle, Exec_Trace haystack); //, Mem_map m);
   ...
   //void getStats (Snapshot& s, unsigned len);
...
private:
   ...
   sigc::connection oTIE_RJF_connection; // onTranlateInstructionEnd
   bool oTIE_connected;
