...
typedef std::vector<data_instance> Reg_Write_Trace;
...
public:
   struct Cfg_ {
      ...
      std::vector<Chunk>     chunks;
		std::vector<symbVar_t> symb_vars;
		std::vector<symbVar_t> monitor_vars;  
   ...
   void initialize ();
	...
	klee::ref<klee::Expr> readMemoryByte (S2EExecutionState* state, uint64_t addr);
   ...
   */
   ...
   void onTranslateJumpStart  (ExecutionSignal* signal, S2EExecutionState* state, TranslationBlock* tb, uint64_t pc, int jump_type);
	...
	void                  onSilentConcretize (S2EExecutionState* state, uint64_t concretized_byte_addr, uint8_t concrete_val, const char* reason);
   ...
   void initDataMap                 (S2EExecutionState* state);
	void addRegWrite                 (uint64_t pc, uint8_t reg, uint64_t offset, uint64_t seq_num, Reg_Write_Trace &trace);
