...
void DasosPreproc::onTranslateInstructionEnd (ExecutionSignal *signal, S2EExecutionState* state, TranslationBlock *tb, uint64_t pc) {
   ...
   // at some point it can go into the kernel (OOB, but then the CR3 is changed to another procs, thus getPid fails to catch further OOB and will not kill a hung observed proc 
   // every time there is a legit OOB or IOB insns, kernel_insns is reset to 0
   if(plgState->kernel_insns > 0) {
      plgState->kernel_insns++;
      if (plgState->out_range_insns < 10) {
         s2e()->getWarningsStream (state) << ",\n";
      }
      else if (plgState->out_range_insns < 100) {
         if (plgState->out_range_insns % 10 == 0) {
            s2e()->getWarningsStream (state) << ".\n";
         }
      }
      else if (plgState->out_range_insns < 1000) {
         if (plgState->out_range_insns % 100 == 0) {
            s2e()->getWarningsStream (state) << ";\n";
         }
      }
      else if (plgState->kernel_insns < 10000) {
         if (plgState->kernel_insns % 1000 == 0) {
            s2e()->getWarningsStream (state) << ":\n";
         }
      }
      else if (plgState->kernel_insns < 100000) {
         if (plgState->kernel_insns % 10000 == 0) {
            s2e()->getWarningsStream (state) << "!\n";
         }
      }
      else if (plgState->kernel_insns < 1000000) {
         if (plgState->kernel_insns % 100000 == 0) {
            s2e()->getWarningsStream (state) << "'\n";
         }
      }
      else if (plgState->kernel_insns % 1000000 == 0) {
         s2e()->getWarningsStream (state) << "\"\n";
      }
   ...
   // this function needs to properly filter out instructions that don't belong to the process we are observing
   // see if not PID
   uint64_t pid = state->getPid();
   if (pid != cfg.proc_id) {
      //s2e()->getWarningsStream (state) << "ignore this insn it is not from the pid we want to observe " << std::hex << " at addr 0x" << pc << " from pid: " << std::dec << pid << std::endl;
      return;
   }
   ...
   if (isInShell (pc) ) {
      if (!plgState->within_range) {
         if (plgState->has_entered_range) {
            s2e()->getWarningsStream (state) << "@0x" << std::hex << pc << ", re-entered buffer range after " << std::dec << plgState->out_range_insns << " OoB insns; last OoB insn @0x" << std::hex << plgState->trace.insns.back().pc + cfg.base_addr << std::dec << ", disasm in debug.\n";
      ...
      // if the call back is activated, but haven't entered range once, then we are just starting out
      // ie at the code between _init/activateModule and the call to the shellcode so ignore this insn
      if (!plgState->has_entered_range) {
         return;
      }
      
      // if the call back is happening bc the kernel has interrupted our proccess without being called by our process
      if (pc >= 0xc0000000) {
      ...
      // at this point it is activated, and has entered the range at least once
      // ie jumped to OOB so tell the debug about this, plgState->trace.insns.back() should be a jmp/call
      if (plgState->within_range) {
         s2e()->getWarningsStream (state) << "@0x" << std::hex << pc << ", left buffer range after " << std::dec << plgState->in_range_insns << " IoB insns; last IoB insn @0x" << std::hex << plgState->trace.insns.back().pc + cfg.base_addr << std::dec << ", disasm in debug.\n";
   ...
   // if has once been in the shell at least once, is currently out of it
   // out of range insns that occur after the range has been entered at least once are handled differently
   if (!plgState->within_range) {
      ...
      // the pc and len are stored, but the bytes values are not, also they don't affect statistics
      ...
      else*/ if (plgState->out_range_insns < 10000) {
         if (plgState->out_range_insns % 1000 == 0) {
            s2e()->getWarningsStream (state) << ":\n";
         }
      }
      else if (plgState->out_range_insns < 100000) {
         if (plgState->out_range_insns % 10000 == 0) {
            s2e()->getWarningsStream (state) << "!\n";
         }
      ...
      else if (plgState->out_range_insns < 1000000) {
         if (plgState->out_range_insns % 100000 == 0) {
            s2e()->getWarningsStream (state) << "'\n";
         }
      }
      else if (plgState->out_range_insns % 1000000 == 0) {
         s2e()->getWarningsStream (state) << "\"\n";
      }
      // all further code is only used if within_range, so leave now
      return;
   }
   ...
   if (plgState->in_range_insns > MAX_IN_RANGE_INSNS) {
      s2e()->getWarningsStream (state) << "!! Potential inifinite loop or wandering execution exceeding MAX_IN_RANGE, caught at 0x" << std::hex << pc << std::endl;
   ...
   if (isFirstInsn && isInsnImpossibleFirst (insn_raw, insn_raw_len) ) {
      ...
      s2e()->getDebugStream() << std::dec << std::setw (3) << 0 << " " << std::setw(3) << insn_raw_len << "B @0x" << std::hex << pc << ":";
   ...
   //printInsn_instance (state, plgState->trace.insns.size () - 1, false);
...
void DasosPreproc::onSyscall (S2EExecutionState* state, uint64_t pc, LinuxSyscallMonitor::SyscallType sysc_type, uint32_t sysc_number, LinuxSyscallMonitor::SyscallReturnSignal& returnsignal) {
   ...
   stream << ">> EIP Found. Syscall number " << std::hex << sysc_number << " at addr 0x" << pc << " offset from base: " << std::dec << (pc - cfg.base_addr) << " (should be EIP-2) within pid: " << pid << " number of instructions: " << plgState->trace.insns.size () << ". This is success #" << cfg.successes.size () + 1 << "\n";
...
void DasosPreproc::printSuccess (struct Success s) {
   //TODO s2e()->getDebugStream() << ">> Success from offset " << s.offset << "\n";
...
void DasosPreproc::printOOBInsn (struct insn_instance insn, unsigned idx, S2EExecutionState* state) {
   ...
   if (!state->readMemoryConcrete (insn.pc + cfg.base_addr, raw, insn.len) ) {
      s2e()->getWarningsStream (state) << "ERROR: could not read guest memory @0x" << std::hex << insn.pc + cfg.base_addr << " to gather ASM insns\n";
      terminateStateEarly_wrap (state, std::string ("eliminated a state with an invalid read") );
      return;
   }
   s2e()->getDebugStream() << std::dec << std::setw (3) << idx << " " << std::setw(3) << insn.len << "B @0x" << std::hex << (insn.pc + cfg.base_addr) << ":";
