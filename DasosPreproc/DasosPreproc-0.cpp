...
void DasosPreproc::onCustomInstruction (S2EExecutionState* state, uint64_t opcode) {
   ...
   switch (op) {
      case 1:
         ...
         break;
      case 2:
         //static inline void s2e_dasospreproc_fuzz (unsigned int* datum, unsigned int start, unsigned int end)
         // time to start fuzzing a particular variable
         // eax = ptr to variable to fuzz
         // ebx = initial value
         // ecx = end value
         
         // maybe hav etemp nonptr written to, then copy that into the ptrssss
         /*ok &= state->readCpuRegisterConcrete(CPU_OFFSET(regs[R_EAX]), &(fuzz_cfg.ptr_addr), 4);
         fuzz_cfg.ptr_addr = fuzz_cfg.ptr_addr & 0xffffffff;
         if (!ok) s2e()->getWarningsStream (state) << "ERROR: bad argument was passed to s2e_op: ptr 0x" << std::hex << fuzz_cfg.ptr_addr << " in DasosPreproc start fuzzing" << std::endl;
         //fuzz_cfg.ptr = (uint64_t*) fuzz_cfg.ptr_addr; // this should handle type issues and convert the input into a pointer*/
         ...
         //s2e()->getWarningsStream (state) 
            //<< "fuzzInit: datum@0x" << std::hex << *(fuzz_cfg.ptr) << std::dec << " to be iterated from " << fuzz_cfg.start << " to " << fuzz_cfg.end << std::endl; 
         ...
         // preserve this state to fork others off of
...
void DasosPreproc::fuzzNext () {
   /* unsigned* ptr;      // set in onCustomInsn
      unsigned orig_val;  // not used right now, but should be set by the onCustomInsn hook
      unsigned start;     // set in onCustomInsn
      unsigned end;       // set in onCustomInsn
    */
...
void DasosPreproc::fuzzClone () {
   // don't forget that if something was malloc'ed that gets written over in here to free it first
   
   /** See if copy constructor works */
   //fuzz_cfg.curr_state = new S2EExecutionState (*(fuzz_cfg.orig_state) );
   // did not work. It ran the "cloned" state and when execution was complete s2e claimed all states were killed and exited as if no additional states were made 
   
   /** Try one of the clone fns. */
   // There one in an S2EExecutionState.cpp , but it is protected. I made a copy of it public and return the proper type. S2EExecutionState* S2EExecutionState::clone_rjf() 
   //fuzz_cfg.curr_state = s2e()->getExecutor()->clone_rjf ();
   // I got a seg fault in the TLB clone code, all members are protected so to deep to move around
   
   /** Try emulating a conditional fork */
   // Called on fork, used to trace forks: StatePair fork(klee::ExecutionState &current, klee::ref<klee::Expr> condition, bool isInternal);
   //forkStates(S2EExecutionState *state, std::vector<S2EExecutionState*> &result, int count, const std::string &varName)


   //s2e()->getDebugStream () << "fuzzClone: 1 " << std::endl;
   /** Try emulating a conditional fork via WindowsApi Code */
   ...
   klee::ref<klee::Expr> cond = klee::NeExpr::create (symb, klee::ConstantExpr::create (0, klee::Expr::Int32) );
   ...
   fuzz_cfg.orig_state = ts; // unless these should be switched...
   ...
   // note that the original function returned symb, is that necessary?
   // here is the error output when I run the above: qemu: /mnt/RJFDasos/s2e/s2e/klee/lib/Core/PTree.cpp:32: std::pair<klee::PTreeNode*, klee::PTreeNode*> klee::PTree::split(klee::PTreeNode*, klee::ExecutionState* const&, klee::ExecutionState* const&): Assertion `n->active && (!n->parent || n->parent->active)' failed.
