...
void DasosPreproc::onTransIOBInsns (S2EExecutionState* state, uint64_t pc, TranslationBlock *tb) {
   ...
   if (isFirstInsn && isInsnImpossibleFirst (insn_raw, insn_raw_len) ) {
      ...
      s2e()->getDebugStream() << std::dec << std::setw (3) << 0 << " " << std::setw(3) << insn_raw_len << "B @0x" << std::noshowbase << std::setw(8) << std::setfill('0') << std::hex << pc << ":";
...
void DasosPreproc::onTranslateInstructionEnd (ExecutionSignal *signal, S2EExecutionState* state, TranslationBlock *tb, uint64_t pc) {
   ...
   //if (plgState->kernel_insns > 3000) s2e()->getWarningsStream (state) << "k:!1(" << plgState->kernel_insns << ")\n";
   ...
   // if it's not a kernel insn, then reset the kernel_insns counter
   ...
   if (pid != cfg.proc_id) {
      ...
      if (plgState->has_entered_range) {
         ...
         s2e()->getWarningsStream (state) << "ignore this insn it is not from the pid we want to observe " << std::hex << " at addr 0x" << std::noshowbase << std::setw(8) << std::setfill('0') << pc << " from pid: " << std::dec << pid << " v goal-pid: " << cfg.proc_id << "\n";
      }
      return;
   }
...
void DasosPreproc::onSyscall (S2EExecutionState* state, uint64_t pc, LinuxSyscallMonitor::SyscallType sysc_type, uint32_t sysc_number, LinuxSyscallMonitor::SyscallReturnSignal& returnsignal) {
   ...
   if (cfg.eip_valid && pc != (cfg.eip_addr - lenOfInsn) ) {
      ...
      stream << "!! Wrong syscall insn found in memory range. It's postload, pid, in range, yet not eip-2, syscall " << std::hex << sysc_number << " at addr 0x" << std::noshowbase << std::setw(8) << std::setfill('0') << pc << " from pid: " << std::dec << pid << std::endl;
   ...
   if (sysc_number > MAX_SYSCALL_NUM) {
      stream << "!! Wrong syscall number makes no sense (>" << MAX_SYSCALL_NUM << ") " << sysc_number << ":0x" << std::noshowbase << std::setw(3) << std::setfill('0') << std::hex << sysc_number << std::endl;
...
void DasosPreproc::printOOBInsn (struct insn_instance insn, unsigned idx, S2EExecutionState* state) {
   ...
   s2e()->getDebugStream() << std::dec << std::setw (3) << idx << " " << std::setw(3) << insn.len << "B @0x" << std::noshowbase << std::setw(8) << std::setfill('0') << std::hex << (insn.pc + cfg.base_addr) << ":";
...
void DasosPreproc::printInsn_instance (struct insn_instance insn, Mem_map m, unsigned idx, bool doDisasm) {
   s2e()->getDebugStream() << std::dec << std::setw (3) << (idx + 1) << " " << std::setw(3) << insn.len << "B @0x" << std::noshowbase << std::setw(8) << std::setfill('0') << std::hex << (insn.pc + cfg.base_addr) << ":";
