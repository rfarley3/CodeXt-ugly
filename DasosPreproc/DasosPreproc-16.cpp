...
void DasosPreproc::initialize() {
   ...
   //cfg.has_forked  = false;
	cfg.clusterWritesBy = s2e()->getConfig()->getInt(getConfigKey() + ".clusterWritesBy", CLUSTER_WRITES_BY);
   s2e()->getDebugStream () << ">> luaLoad: clusterWritesBy " << cfg.clusterWritesBy << "\n"; 
	cfg.minExecInsns    = s2e()->getConfig()->getInt(getConfigKey() + ".minExecInsns",    MIN_EXEC_INSNS);
   s2e()->getDebugStream () << ">> luaLoad: minExecInsns " << cfg.minExecInsns << "\n"; 
	cfg.minExecBytes    = s2e()->getConfig()->getInt(getConfigKey() + ".minExecBytes",    MIN_EXEC_BYTES);
   s2e()->getDebugStream () << ">> luaLoad: minExecBytes " << cfg.minExecBytes << "\n"; 
	cfg.maxInRangeInsn  = s2e()->getConfig()->getInt(getConfigKey() + ".maxInRangeInsn",  MAX_IN_RANGE_INSNS);
   s2e()->getDebugStream () << ">> luaLoad: maxInRangeInsn " << cfg.maxInRangeInsn << "\n"; 
	cfg.maxOutRangeInsn = s2e()->getConfig()->getInt(getConfigKey() + ".maxOutRangeInsn", MAX_OUT_RANGE_INSNS);
   s2e()->getDebugStream () << ">> luaLoad: maxOutRangeInsn " << cfg.maxOutRangeInsn << "\n"; 
	cfg.maxKernelInsn   = s2e()->getConfig()->getInt(getConfigKey() + ".maxKernelInsn",   MAX_KERNEL_INSNS);
   s2e()->getDebugStream () << ">> luaLoad: maxKernelInsn " << cfg.maxKernelInsn << "\n"; 
	cfg.maxKillableInsn = s2e()->getConfig()->getInt(getConfigKey() + ".maxKillableInsn", MAX_KILLABLE_INSNS);
   s2e()->getDebugStream () << ">> luaLoad: maxKillableInsn " << cfg.maxKillableInsn << "\n"; 
	
	// get list of memory addresses to make symbolic
	std::vector<std::string> symbData;
	symbData = s2e()->getConfig()->getStringList (getConfigKey() + ".symbVars");  
	for (uint32_t i = 0; i < symbData.size (); i++) {
		struct symbVar_t symbVar;
		symbVar.name = symbData[i]; i++;                 // name of symb var 
		symbVar.addr = atoi (symbData[i].c_str() ); i++; // offset in bytes from start of shellcode loaded
		symbVar.len  = atoi (symbData[i].c_str() ); i++; // length in num bytes of symbolic variable
		symbVar.when = atoi (symbData[i].c_str() );      // at which insn to make symbolic
		symbVar.marked = 0;
		cfg.symbVars.push_back (symbVar);
   	s2e()->getDebugStream () << ">> luaLoad: symbVars[" << int (i/4) << "]: " << symbVar.name << " at " << symbVar.addr << " for " << symbVar.len << "B after insn " << symbVar.when << "\n"; 
	}
	
	
	// get list of memory addresses to watch
	//std::vector<std::string> symbData;
	symbData = s2e()->getConfig()->getStringList (getConfigKey() + ".monitorVars");  
	for (uint32_t i = 0; i < symbData.size (); i++) {
		struct symbVar_t symbVar;
		symbVar.name = symbData[i]; i++;                 // name of var to watch, not used by program, only useful to user
		symbVar.addr = atoi (symbData[i].c_str() ); i++; // offset in bytes from start of shellcode loaded
		symbVar.len  = atoi (symbData[i].c_str() ); i++; // length in num bytes of variable
		symbVar.when = atoi (symbData[i].c_str() );      // at which insn to start monitoring
		symbVar.marked = 0;
		cfg.monitorVars.push_back (symbVar);
   	s2e()->getDebugStream () << ">> luaLoad: monitorVars[" << int (i/4) << "]: " << symbVar.name << " at " << symbVar.addr << " for " << symbVar.len << "B after insn " << symbVar.when << "\n"; 
	}
	...
	cfg.multiSysc   = s2e()->getConfig()->getBool (getConfigKey() + ".multiPath", false); 
   s2e()->getDebugStream () << ">> luaLoad: multisysc " << (cfg.multiSysc ? "true" : "false") << "\n"; 
...
void DasosPreproc::onTransIOBInsns (ExecutionSignal* signal, S2EExecutionState* state, TranslationBlock* tb, uint64_t pc, uint64_t len, uint64_t nextpc) {
   ...
   if (!state->readMemoryConcrete (pc, insn_raw, len) ) {
      s2e()->getWarningsStream (state) << "!! ERROR: could not read guest memory @0x" << std::noshowbase << std::setw(8) << std::setfill('0') << std::hex << pc << " to gather ASM insns, oTIOBI\n";
...
void DasosPreproc::onExecuteInsn (S2EExecutionState* state, uint64_t pc) {
   ...
   uint8_t insn_raw[e.len];
	for (unsigned i = 0; i < e.len; i++) {
   	if (!state->readMemoryConcrete (pc + i, &(insn_raw[i]), 1) ) {
      	s2e()->getWarningsStream (state) << " >> WARNING: this byte is symbolic, code doesn't exist to convert to a valid concrete value @0x" << std::noshowbase << std::setw(8) << std::setfill('0') << std::hex << pc << " needed to gather ASM insns, oEI\n";
   ...
   if (0) {
      ...
      /*if(!m_runningConcrete || !m_cpuRegistersObject->isConcrete(offsetof(CPUState, regs[R_EAX]), klee::Expr::Int32)) {
         s2e()->getMessagesStream(state) << "Reg EAX is symbolic, expr: " << m_cpuRegistersObject->read(offsetof(CPUState, regs[R_EAX]), klee::Expr::Int32) << "\n";
      } 
      else {
         uint64_t ret = 0;
         small_memcpy((void*) &ret, (void*) (m_cpuRegistersState->address + offsetof(CPUState, regs[R_EAX])), klee::Expr::getMinBytesForWidth(klee::Expr::Int32));
         symb_val = klee::ConstantExpr::create(ret, klee::Expr::Int32);
         // this should be getZExtValue(32) to read entire register
      ...
      if (isSymb_extended (state, z_addr, conc_val, symb_val) ) {
         ...
         //s2e()->getMessagesStream(state) << "Z (0x" << std::hex << std::setw(8) << std::setfill('0') << z_addr << ") is symbolic simplified, expr: " << state->constraints.simplifyExpr (symb_val) << "\n";
      ...
      if (!plgState->x_is_symb) { //(plgState->execed_insns == 1) {
         ...
         markExecutableSymb (state, x_addr, "x");
         plgState->x_is_symb = true;
      }
   } // end if avalance addition tester
	else if (1) {  // now try to execute code that you've marked as symbolic
		// avalance XOR using symbVars and monitorVars
      // use with ./shellcode-wrapper -i hw-xor.rawshell -x -f 0 -c 1
      uint8_t conc_val;
      klee::ref<klee::Expr> symb_val;
		for (uint32_t i = 0; i < cfg.symbVars.size (); i++) {
			s2e()->getMessagesStream(state) << cfg.symbVars[i].name << " (0x" << std::hex << std::setw(8) << std::setfill('0') << cfg.base_addr + cfg.symbVars[i].addr << ") ";
	      if (isSymb_extended (state, cfg.base_addr + cfg.symbVars[i].addr, conc_val, symb_val) ) {
	         s2e()->getMessagesStream(state) << "is symbolic, expr: " << symb_val << "\n";
	         //printSymb (state, key_addr);
	      }
	      else {
	         s2e()->getMessagesStream(state) << "is concrete, val:  0x" << std::hex << std::setw(2) << std::setfill('0') << (unsigned int) conc_val << "; expr: " << symb_val << "\n";
				if (!cfg.symbVars[i].marked && plgState->execed_insns >= cfg.symbVars[i].when) {
		         markSymb (state, cfg.symbVars[i]);
					cfg.symbVars[i].marked = 1;
				}
	      }
		}
		for (uint32_t i = 0; i < cfg.monitorVars.size (); i++) {
			s2e()->getMessagesStream(state) << cfg.monitorVars[i].name << " (0x" << std::hex << std::setw(8) << std::setfill('0') << cfg.base_addr + cfg.monitorVars[i].addr << ") ";
	      if (isSymb_extended (state, cfg.base_addr + cfg.monitorVars[i].addr, conc_val, symb_val) ) {
	         s2e()->getMessagesStream(state) << "is symbolic, expr: " << symb_val << "\n";
	         //printSymb (state, key_addr);
	      }
	      else {
      ...
      uint8_t conc_val;
      klee::ref<klee::Expr> symb_val;
      // is key symb?  //12 is the asm code, 13 is the 1st B of the key
      uint64_t key_addr = cfg.base_addr + 13;
      ...
      if (!plgState->x_is_symb) {
         markExecutableSymb (state, key_addr, "key");
         plgState->x_is_symb = true;
      }
   }
   else if (0) {
      ...
      //if (pc >= (cfg.base_addr + 0x3e) ) { 
         ...
         if (isSymb_extended (state, x2_addr, conc_val, symb_val) ) {
            s2e()->getMessagesStream(state) << "X (0x" << std::hex << std::setw(8) << std::setfill('0') << x2_addr << ") is symbolic, expr: " << symb_val << "\n";
            //printSymb (state, x_addr);
         }
         else {
            s2e()->getMessagesStream(state) << "X (0x" << std::hex << std::setw(8) << std::setfill('0') << x2_addr << ") is concrete, val:  0x" << std::hex << std::setw(2) << std::setfill('0') << (unsigned int) conc_val << "; expr: " << symb_val << "\n";
         }
      }
   }
   return;
...
void DasosPreproc::markSymb (S2EExecutionState* state, struct symbVar_t svar) {
	for (uint32_t i = 0; i < svar.len; i++) {
		//markSymb (state, cfg.base_addr + svar.addr + i, svar.name);
		markExecutableSymb (state, cfg.base_addr + svar.addr + i, svar.name);
	}
	return;
} // end fn markSymb
...
void DasosPreproc::markSymb (S2EExecutionState* state, uint32_t address, std::string nameStr) {
   ...
   s2e()->getMessagesStream(state)
      << "Inserting fully symbolic byte at 0x" << std::hex << address
...
void DasosPreproc::markExecutableSymb (S2EExecutionState* state, uint32_t address, std::string nameStr) {
   char buf; // consider typing as uint8_t for consistency
   //klee::ref<klee::Expr> symb_val;
   ...
   /*// turn symb_const into (symb_const + symb_taint): (Add w8 (w8 <value in memory>) (Read w8 0 <taint name>) )
    k lee::ref<klee::E*xpr> symb_taint = state->createSymbolicValue (klee::Expr::Int8, nameStr);
...
void DasosPreproc::onSyscall (S2EExecutionState* state, uint64_t pc, uint32_t sysc_number) {
   ...
   if (plgState->exec_trace.insns.size() < cfg.minExecInsns) { 
   ...
   for (unsigned i = 0; i < d.writes.size (); i++) {
      if (d.writes[i].in_range) {
         ...
         for (unsigned j = 0; j < d.writes[i].bytes.size (); j++) {
            ...
            else if (m.size () > 1 && (d.writes[i].seq_num - last_write_seq_num) >= cfg.clusterWritesBy) { // 10
               diff_cluster = true;
            }
...
void DasosPreproc::printOOBInsn (S2EExecutionState* state, trans_instance insn, unsigned num_oob) {
   ...
   if (!state->readMemoryConcrete (insn.addr + cfg.base_addr, raw, insn.len) ) {
      s2e()->getWarningsStream (state) << "!! ERROR: could not read guest memory @0x" << std::noshowbase << std::setw(8) << std::setfill('0') << std::hex << insn.addr + cfg.base_addr << " to gather ASM insns, pOOBI\n";
      terminateStateEarly_wrap (state, std::string ("eliminated a state with an invalid read"), true);
      return;
   }
