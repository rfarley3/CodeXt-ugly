  ...
  case Instruction::Br: {
    ...
    if (bi->isUnconditional()) {
      ...
      if (branches.second)
		...
		notifyFork(state, cond, branches); // Vitaly
    }
    break;
  }
  ...
  case Instruction::Call: {
    ...
    if (f) {
      ...
      do {
        ...
        assert(success && "FIXME: Unhandled solver failure");
        (void) success;
        //StatePair res = fork(*free, EqExpr::create(v, value), true);// Vitaly
        ref<Expr> cond = EqExpr::create(v, value); // Vitaly
		  StatePair res = fork(*free, cond, true); // Vitaly
		  notifyFork(*free, cond, res); // Vitaly
        if (res.first) {
...
void Executor::executeAlloc(ExecutionState &state,
  ...
  if (ConstantExpr *CE = dyn_cast<ConstantExpr>(size)) {
    ...
    //StatePair fixedSize = fork(state, EqExpr::create(example, size), true); // Vitaly
    ref<Expr> cond = EqExpr::create(example, size); // Vitaly
    StatePair fixedSize = fork(state, cond, true); // Vitaly
    notifyFork(state, cond, fixedSize); // Vitaly
...
void Executor::executeFree(ExecutionState &state,
                           ref<Expr> address,
                           KInstruction *target) {
  //StatePair zeroPointer = fork(state, Expr::createIsZero(address), true); // Vitaly
  ref<Expr> cond = Expr::createIsZero(address); // Vitaly
  StatePair zeroPointer = fork(state, cond, true); // Vitaly
  ...
  notifyFork(state, cond, zeroPointer); // Vitaly
}
...
void Executor::resolveExact(ExecutionState &state,
  ...
  for (ResolutionList::iterator it = rl.begin(), ie = rl.end(); 
    ...
    StatePair branches = fork(*unbound, inBounds, true);
  	 notifyFork(*unbound, inBounds, branches); // Vitaly
...
void Executor::executeMemoryOperation(ExecutionState &state,
  ...
  for (ResolutionList::iterator i = rl.begin(), ie = rl.end(); i != ie; ++i) {
    ...
    if (bound) {
	 ...
	 notifyFork(state, inBounds, branches); // Vitaly
    unbound = branches.second;
    if (!unbound)
      break;
  }
