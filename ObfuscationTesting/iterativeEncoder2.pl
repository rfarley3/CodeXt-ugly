...
my $key_int = 0x44332211;  # TODO use int to begin with and convert/unpack when substituting into decoder
$decoder =~ s/KEY/$key/;
...
for (my $i = 0; $i < $frag_len; $i++) {
   ...
   print "\n";
   # notice that encode can return a modified key
   ($frag[$i], $key_int) = encode ($frag[$i], $key_int);
   #$key_int = modifyKey ($key_int, $last_word);
   print "             Enc $i: ";
...
sub encode {
   my ($plaintxt, $key_int) = @_;
   my $new_key = 0;
   my @plain_words = unpack ('l*', $plaintxt);
   my @cipher_words = ();
   for (my $i = $#plain_words; $i >= 0; $i--) {
      my $cipher_word = $key_int ^ $plain_words[$i];
      unshift (@cipher_words, $cipher_word);
      $new_key = ($key_int + $plain_words[$i]) & 0xffffffff;
   }
   my $ciphertxt = pack ('l*', @cipher_words);
   #return ($ciphertxt, $key_int); # use this for non-mutating key
   return ($ciphertxt, $new_key);
} # end sub encode

sub modifyKey {
   my ($key_in, $word) = @_;
   #my $key_out = $key_in + $word;
   # optionally modify the key for the next fragment's encoding
   # NOTE the decoder needs a complimentary key modifier
   # the decoder will have access to the decoded value (plain_word), so use that to modify the key
   # decoder will need (assuming key is in ebx, it's a countdown where edi is the base (so edi+(ecx-1)*4=edi after loop) this insn: add ebx,[edi]
   #my $key_int = $key + $last_plain_word;
   my $key_out = $key_in;
   #$key_out = sprintf("%d", int($key_in) );
   #unpack ('l' , $key_in ); 
   #print "ki: $key_int\n";
   #my $plain_int = pack ('l', $last_plain_word );
   #print "pi: $plain_int\n";
   #$key_int += $plain_int;
   #print "new_ki: $key_int\n";
   #$key = pack ('W', int ($key_int) );
   #print "  Key:       ";
   #my $key_hex = unpack('H*', $key);
   #for (my $i = 0; $i < length $key_hex; $i+=2) {
   #   if($i % 8 eq 0){ print " ";}
