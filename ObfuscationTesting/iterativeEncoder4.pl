...
my $key = rand (0xffffffff); #0x44332211;  # TODO randomize key
...
sub encode {
   ...
   for (my $i = $#plain_words; $i >= 0; $i--) {
      ...
      #$new_key = (int($key_int) + int($plain_words[$i])) & 0xffffffff;
   }
   #$new_key = addX86 ($key_int, $plain_words[0]);
   $new_key = $key + $plain_words[0];
   # invert byte order
   #$new_key = unpack('I>!', pack ('I<!', $new_key) );
   #$new_key = addX86 (pack ('I>!', unpack ('c', $key_int) ), pack ('I>!', unpack ('c', $plain_words[0]) ) );
...
sub addX86 {
   ...
   foreach my $n (@_) {
      ...
      my $x=unpack('I>!',$s);
      #printf("3:%#x\n",$x);
      # for older perl: my $x = unpack ('N', pack ('V', $n) );

      # convert to binary to see if it is a negative number
      my $b = sprintf "%#b", ($x & 0xffffffff);
      my $len = length ($b);
      while ($len < 34) {
         $b =~ s/^0b/0b0/;
         $len++;
      ...
      #print "adding $b\n";
      # manually set its negative value if negative
      if ($b =~ /^0b1/) {
         $b =~ s/^0b1/0b0/; 
         $x = sprintf "%d", (-2147483648 + oct ($b) );
      }
      # otherwise its value is correct (equiv to unsigned)
      else {
         $x = sprintf "%d", oct ($b);
      }

      #my $h = sprintf "%#x", ($x & 0xffffffff);
      #print "which is signed $x : $h\n";
      # add it
      $res += $x;
   }
   # invert byte order
