...
my $key_int = int("\x11\x22\x33\x44");
$decoder =~ s/KEY/$key/;
...
sub encode {
   ...
   my $ciphertxt = "" ;
   my $new_key = 0;
   # work from the end just like the decoder
   for (my $i = (length ($plaintxt) - 1); $i >= 0; $i-=4) {
      my $plain_word = int (substr ($plaintxt, $i, 4) );
      my $cipher_word = $key_int ^ $plain_word;
      $new_key = $key_int + $plain_word;
      $ciphertxt .= $cipher_word; 
   }
   return ($ciphertxt, $key_int); 
