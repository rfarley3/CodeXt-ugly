...
public:
   struct Cfg_ {
      ...
      uint64_t byte_len;
		bool     len_valid;
		bool     elf_mode;
		uint64_t elf_loaded_sig;
      bool     elf_label_netw_in;
   ...
   /* Run when the plugin is loaded, read conf.lua
	*/
   void initialize ();
   ...
   // sets up the buffer boundaries
   void onActivateModule (S2EExecutionState* state);
	// set up all the hooks (oDMA, oTIE, oTBE, oTBS, oPC, oExc, oPF, oTJS)
	void setHooks (S2EExecutionState* state);
   ...
   void terminateStateEarly_wrap (S2EExecutionState* state, std::string msg, bool possible_success);
	...
	void onFiniElf (S2EExecutionState* state);
   ...
   bool isInShell (S2EExecutionState* state, uint64_t pc);      
   ...
   */
	/* 0x00000000 for 4K zero page
	 * 0x00001000 for 128M not used
	 * 0x08000000 for 896M app code/data follow by small malloc space
	 * 0x40000000 for 1G mmap, lib load, large malloc
	 * 0x80000000 for 1G stack working back from bfffffe0  
	 */
	// the elf loader like to spend a lot of time in 0xb7*
	bool isInLoaderSegment (uint64_t pc);
	
	// register offsets (per klee/qemu) accessors and name<->offset converters
   ...
   bool                  isSymb_ext    (S2EExecutionState* state, uint32_t address, uint8_t& conc_val, klee::ref<klee::Expr>& symb_val);
   ...
   void onSyscall         (S2EExecutionState* state, uint64_t pc, uint32_t sysc_number);
	// is sysc_number an "end of path" value, ie if this is non-multi-modeling, should we end here
   ...
   bool isInNormalizeMode (S2EExecutionState* state); // is this a non-forking run (state0 got a syscall IOB)
	...
	// Success (a type of Fragment); and Fragment management
   ...
   // symb_var_t, user specified variables to either mark as symb or simply monitor
	struct symb_var_t getUserVar      (std::vector<std::string> symb_data, unsigned& i);
	...
	/* Begin Taint Mechanism fns */
   // core functional components to the taint mechanism
   void                                 markNetwInSymb         (S2EExecutionState* state, uint32_t buf_addr, uint32_t buf_len);
   ...
   struct ConstraintExpr       getConstraint (klee::ref<klee::Expr> c);
	/* Klee Expr manipulators/helpers */
	// Simplify and its helpers
	// this traverses an expr tree and extracts only the labels that should exists, then solves the equation (into its simplest form, a ConstantExpr) and relabels it
	void                                 simplifyMemory         (S2EExecutionState* state); // iterates through all memory
	void                                 simplifyAddr           (S2EExecutionState* state, uint64_t pc, bool is_reg); // called per byte
   ...
   // we often collect labels, this only adds labels that don't already exist in the output (unique add)
	...
	// traverses an expr, using unique add to collect labels
	std::vector< klee::ref<klee::Expr> > getLabels              (klee::ref<klee::Expr> e);
   // convert a Taint Label Expr into a string
	std::string                          getLabelStr            (klee::ref<klee::Expr> e);
   std::string                          getBaseLabelStr        (std::string s);
	std::string                          trimLabelStr           (std::string s);
	// some means to keep expr as simple to read as possible
	...
	/* End of Klee and Taint Mechanism */
   ...
   void  printWriteInstance    (data_instance w);
	void  setSyscNames          ();
	std::string printSyscName   (unsigned int num);
   ...
   // Raw and Disassembly Printing/Accessors
   ...
   void fuzzForkSingle           (S2EExecutionState* state, unsigned int value);
	bool isaFuzzForkConstraint    (klee::ref<klee::Expr> e);
	void purgeFuzzForkConstraints (S2EExecutionState* state);
};
...
private:
   ...
   bool             oSC_connected;
	sigc::connection oSF_connection;      // onStateFork
