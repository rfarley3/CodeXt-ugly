...
int main (int argc, char* argv[]) {
   ...
   for (combo = 0; combo < combos; combo++) {
      ...
      for (i = 0; i < buf_its[combo]; i++) {
         ...
         // output file
         char filename[256];
         snprintf (filename, sizeof (filename), "randfill-%ukb:%u%s.rawshell", buf_sizes[combo], i, sysc_found ? "-cd80" : "");
         FILE* out;
         if ((out = fopen (filename, "w") ) == NULL) {
            printf ("error: couldn't open file %s\n", filename);
            exit (0);
         }
         if (fwrite (out, sizeof (uint8_t), buf_sizes[combo], buf) != buf_sizes[combo]) {
